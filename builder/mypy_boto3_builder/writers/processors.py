...
def process_boto3_stubs(
    ...
    modified_paths = write_boto3_stubs_module(boto3_module, output_path, reformat)
...
def process_master(
    ...
    modified_paths = write_master_module(master_module, output_path, reformat)
    for modified_path in modified_paths:
        logger.debug(f"Updated {NicePath(modified_path)}")
...
def process_service(
    session: Session, service_name: ServiceName, output_path: Path, reformat: bool
) -> ServiceModule:
    ...
    modified_paths = write_service_module(service_module, output_path, reformat)
    for modified_path in modified_paths:
        logger.debug(f"Updated {NicePath(modified_path)}")
