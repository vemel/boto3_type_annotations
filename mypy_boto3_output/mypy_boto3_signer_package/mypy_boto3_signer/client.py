...
class Client(BaseClient):
    ...
    def list_signing_jobs(
        ...
        signing returns a ``nextToken`` value. Use this value in subsequent calls to ``ListSigningJobs`` to
        fetch the remaining values. You can continue calling ``ListSigningJobs`` with your ``maxResults``
        parameter and with new values that code signing returns in the ``nextToken`` parameter until all of
