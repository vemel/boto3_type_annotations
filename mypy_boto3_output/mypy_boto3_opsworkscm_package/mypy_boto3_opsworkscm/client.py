...
class Client(BaseClient):
    ...
    def associate_node(
        ...
        **Request Syntax**
        ...
        :param EngineAttributes: **[REQUIRED]**
          ...
          * ``CHEF_ORGANIZATION`` : The Chef organization with which the node is associated. By default
    ...
    def create_server(
        ...
        account. A ``ResourceNotFoundException`` is thrown when you specify a backup ID that is not valid
        or is for a backup that does not exist. A ``ValidationException`` is thrown when parameters of the
        ...
        **Request Syntax**
        ...
        :param EngineAttributes:
          ...
          * ``CHEF_AUTOMATE_ADMIN_PASSWORD`` : The password for the administrative user in the Chef
          ...
          of 32. The password can contain letters, numbers, and special characters (!/@#$%^&+=_). The
          password must contain at least one lower case letter, one upper case letter, one number, and one
          special character. When no CHEF_AUTOMATE_ADMIN_PASSWORD is set, one is generated and returned in
          the response.
        ...
        :param InstanceProfileArn: **[REQUIRED]**
          ...
          console typically creates the instance profile for you, if you are using API commands instead,
          run the service-role-creation.yaml AWS CloudFormation template, located at
        ...
        :param SubnetIds:
          ...
          are created in a default subnet that is selected by Amazon EC2. If you specify subnet IDs, the
    ...
    def delete_backup(self, BackupId: str) -> Dict[str, Any]:
        ...
        ``ResourceNotFoundException`` is thrown when the backup does not exist. A ``ValidationException``
    ...
    def delete_server(self, ServerName: str) -> Dict[str, Any]:
        ...
        instance). When you run this command, the server state is updated to ``DELETING`` . After the
        server is deleted, it is no longer returned by ``DescribeServer`` requests. If the AWS
    ...
    def describe_node_association_status(
        ...
        the specified token is found, or when the server does not exist. A ``ValidationException`` is
    ...
    def restore_server(
        ...
        deleted, and a new EC2 instance is configured. RestoreServer maintains the existing server
        endpoint, so configuration management of the server's client devices (nodes) should continue to
        work.
    ...
    def update_server_engine_attributes(
        ...
        when this operation is in progress. Only one update can occur at a time. You can use this command
        to reset a Chef server's public key (``CHEF_PIVOTAL_KEY`` ) or a Puppet server's admin password
