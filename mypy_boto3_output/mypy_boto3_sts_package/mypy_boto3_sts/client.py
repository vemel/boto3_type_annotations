...
class Client(BaseClient):
    ...
    def assume_role(
        ...
        might not normally have access to. These temporary credentials consist of an access key ID, a
        secret access key, and a security token. Typically, you use ``AssumeRole`` within your account or
        for cross-account access. For a comparison of ``AssumeRole`` with other API operations that produce
        ...
        temporary security credentials to access all the other accounts by assuming roles in those
        ...
        session. You can provide a value from 900 seconds (15 minutes) up to the maximum session duration
        ...
        use for both inline and managed session policies shouldn't exceed 2048 characters. Passing policies
        ...
        * Add the user as a principal directly in the role's trust policy.
        ...
        the role has been authenticated with an AWS MFA device. In that scenario, the trust policy of the
        role being assumed includes a condition that tests for MFA authentication. If the caller does not
        include valid MFA information, the request to assume the role is denied. The condition in a trust
        ...
        **Request Syntax**
        ...
        :param PolicyArns:
          ...
          that you use for both inline and managed session policies shouldn't exceed 2048 characters. For
          more information about ARNs, see `Amazon Resource Names (ARNs) and AWS Service Namespaces
          ...
          .. note::
            ...
            However, an AWS conversion compresses the session policies into a packed binary format that has
            a separate limit. This is the enforced limit. The ``PackedPolicySize`` response element
          ...
          You can use the role's temporary credentials in subsequent AWS API calls to access resources in
          the account that owns the role. You cannot use session policies to grant more permissions than
          those allowed by the identity-based policy of the role that is being assumed. For more
          information, see `Session Policies
          <https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html#policies_session>`__ in
          the *IAM User Guide* .
        ...
        :param Policy:
          ...
          permissions than those allowed by the identity-based policy of the role that is being assumed.
        ...
        :param DurationSeconds:
          ...
          .. note::
            ...
            of the console session. For more information, see `Creating a URL that Enables Federated Users
        ...
        :param TokenCode:
          ...
          requires MFA and if the ``TokenCode`` value is missing or expired, the ``AssumeRole`` call
    ...
    def assume_role_with_saml(
        ...
        **Request Syntax**
        ...
        :param DurationSeconds:
          ...
          setting for the role. This setting can have a value from 1 hour to 12 hours. If you specify a
          value higher than this setting, the operation fails. For example, if you specify a session
    ...
    def assume_role_with_web_identity(
        ...
        or web application with a web identity provider. Example providers include Amazon Cognito, Login
        with Amazon, Facebook, Google, or any OpenID Connect-compatible identity provider.

        .. note::

          For mobile applications, we recommend that you use Amazon Cognito. You can use Amazon Cognito
          with the `AWS SDK for iOS Developer Guide <http://aws.amazon.com/sdkforios/>`__ and the `AWS SDK
          for Android Developer Guide <http://aws.amazon.com/sdkforandroid/>`__ to uniquely identify a
          user. You can also supply the user with a consistent identity throughout the lifetime of an
          application.
        ...
        The temporary security credentials returned by this API consist of an access key ID, a secret
        access key, and a security token. Applications can use these temporary security credentials to sign
        ...
        * `Web Identity Federation Playground
        ...
        process of authenticating through Login with Amazon, Facebook, or Google, getting temporary
        ...
        * `AWS SDK for iOS Developer Guide <http://aws.amazon.com/sdkforios/>`__ and `AWS SDK for Android
        ...
        show how to invoke the identity providers, and then how to use the information from these providers
    ...
    def decode_authorization_message(
        ...
        * Whether the request was denied due to an explicit deny or due to the absence of an explicit
        allow. For more information, see `Determining Whether a Request is Allowed or Denied
    ...
    def get_access_key_info(
        ...
        information about access keys, see `Managing Access Keys for IAM Users
        ...
        keys belong. Access key IDs beginning with ``AKIA`` are long-term credentials for an IAM user or
        the AWS account root user. Access key IDs beginning with ``ASIA`` are temporary credentials that
        are created using STS operations. If the account in the response belongs to you, you can sign in as
        the root user and review your root user access keys. Then, you can pull a `credentials report
        ...
        <https://docs.aws.amazon.com/IAM/latest/UserGuide/cloudtrail-integration.html>`__ .
    ...
    def get_federation_token(
        ...
        temporary security credentials on behalf of distributed applications inside a corporate network.
        You must call the ``GetFederationToken`` operation using the long-term security credentials of an
        IAM user. As a result, this call is appropriate in contexts where those credentials can be safely
        ...
        You can also call ``GetFederationToken`` using the security credentials of an AWS account root
        user, but we do not recommend it. Instead, we recommend that you create an IAM user for the purpose
        of the proxy application. Then attach a policy to the IAM user that limits federated users to only
        the actions and resources that they need to access. For more information, see `IAM Best Practices
        ...
        a maximum of 129,600 seconds (36 hours). The default is 43,200 seconds (12 hours). Temporary
        credentials that are obtained by using AWS account root user credentials have a maximum duration of
        3,600 seconds (1 hour).

        The temporary security credentials created by ``GetFederationToken`` can be used to make API calls
        to any AWS service with the following exceptions:
        ...
        * You cannot call any STS API operations except ``GetCallerIdentity`` .
        ...
        operation. You can pass a single JSON policy document to use as an inline session policy. You can
        also specify up to 10 managed policies to use as managed session policies. The plain text that you
        use for both inline and managed session policies shouldn't exceed 2048 characters.
        ...
        session in the ``Principal`` element of the policy. When you pass session policies, the session
        permissions are the intersection of the IAM user policies and the session policies that you pass.
        This gives you a way to further restrict the permissions for a federated user. You cannot use
        session policies to grant more permissions than those that are defined in the permissions policy of
        ...
        **Request Syntax**
        ...
        :param Policy:
          ...
          this operation. You can pass a single JSON policy document to use as an inline session policy.
          ...
          federated user session has no permissions. The only exception is when the credentials are used to
          access a resource that has a resource-based policy that specifically references the federated
        ...
        :param PolicyArns:
          ...
          <https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html#policies_session>`__ to
          this operation. You can pass a single JSON policy document to use as an inline session policy.
          You can also specify up to 10 managed policies to use as managed session policies. The plain text
          that you use for both inline and managed session policies shouldn't exceed 2048 characters. You
          can provide up to 10 managed policy ARNs. For more information about ARNs, see `Amazon Resource
    ...
    def get_session_token(
        ...
        if you want to use MFA to protect programmatic calls to specific AWS API operations like Amazon EC2
        ...
        based on account credentials can range from 900 seconds (15 minutes) up to 3,600 seconds (1 hour),
