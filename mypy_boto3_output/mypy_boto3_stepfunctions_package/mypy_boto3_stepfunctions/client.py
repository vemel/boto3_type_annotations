...
class Client(BaseClient):
    ...
    def create_activity(
        ...
        .. note::
           ...
           ``CreateActivity`` is an idempotent API. Subsequent requests won’t create a duplicate resource
           if it was already created. ``CreateActivity`` 's idempotency check is based on the activity
           ``name`` . If a following request has different ``tags`` values, Step Functions will ignore
           these differences and treat it as an idempotent request of the previous. In this case, ``tags``
    ...
    def create_state_machine(
        ...
        .. note::
           ...
           state machine ``name`` and ``definition`` . If a following request has a different ``roleArn``
           or ``tags`` , Step Functions will ignore these differences and treat it as an idempotent request
           of the previous. In this case, ``roleArn`` and ``tags`` will not be updated, even if they are
           different.
    ...
    def get_execution_history(
        ...
        **Request Syntax**
        ...
        :param maxResults:
          ...
          further pages of results. The default is 100 and the maximum allowed page size is 1000. A value
    ...
    def start_execution(
        ...
        .. note::
           ...
           as a running execution, the call will succeed and return the same response as the original
           request. If the execution is closed or if the input is different, it will return a 400
