...
class Client(BaseClient):
    ...
    def describe_dataset(
        ...
        identity has access only to its own data. Thus, the credentials used to make this API call need to
        have access to the identity data.
    ...
    def register_device(
        ...
        This API can only be called with temporary credentials provided by Cognito Identity. You cannot
    ...
    # pylint: disable=arguments-differ
    def set_cognito_events(self, IdentityPoolId: str, Events: Dict) -> None:
        """
    ...
    def update_records(
        ...
        reject an UpdateRecords request with a ResourceConflictException if you try to patch a record with
        a new value but a stale sync count.

        For example, if the sync count on the server is 5 for a key called highScore and you try and submit
        a new highScore with sync count of 4, the request will be rejected. To obtain the current sync
        count for a record, call ListRecords. On a successful update of the record, the response returns
        the new sync count for that record. You should present that sync count the next time you try to
        update that same record. When the record does not exist, specify the sync count as 0.
