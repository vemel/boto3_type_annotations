...
class Client(BaseClient):
    ...
    def cancel_rotate_secret(
        ...
        .. note::
          ...
          unexpected state. Depending on what step of the rotation was in progress, you might need to
          remove the staging label ``AWSPENDING`` from the partially created version, specified by the
          ``VersionId`` response value. You should also evaluate the partially rotated new version to see
          if it should be deleted, which you can do by removing all staging labels from the new version's
          ``VersionStage`` field.
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          .. note::
            ...
            If you specify an ARN, we generally recommend that you specify a complete ARN. You can specify
            a partial ARN too—for example, if you don’t include the final hyphen and six random characters
            that Secrets Manager adds at the end of the ARN when you created the secret. A partial ARN
            match can work as long as it uniquely matches only one secret. However, if your secret has a
            name that ends in a hyphen followed by six characters (before Secrets Manager adds the hyphen
            and six characters to the ARN) and you try to use that as a partial ARN, then those characters
            cause Secrets Manager to assume that you’re specifying a complete ARN. This confusion can cause
    ...
    def create_secret(
        ...
        """
        Creates a new secret. A secret in Secrets Manager consists of both the protected secret data and
        ...
        ``SecretString`` or ``SecretBinary`` then Secrets Manager also creates an initial secret version
        and automatically attaches the staging label ``AWSCURRENT`` to the new version.
        ...
        .. note::
          ...
          * If you call an operation that needs to encrypt or decrypt the ``SecretString`` or
          ...
          master key (CMK) with the alias ``aws/secretsmanager`` . If this key doesn't already exist in
          your account then Secrets Manager creates it for you automatically. All users and roles in the
          same AWS account automatically have access to use the default CMK. Note that if an Secrets
          Manager API call results in AWS having to create the account's AWS-managed CMK, it can result in
        ...
        * kms:GenerateDataKey - needed only if you use a customer-managed AWS KMS key to encrypt the
        secret. You do not need this permission to use the account's default AWS managed CMK for Secrets
        Manager.
        ...
        **Request Syntax**
        ...
        :param ClientRequestToken:
          ...
          .. note::
            ...
            parameter empty. The CLI or SDK generates a random UUID for you and includes it as the value
            for this parameter in the request. If you don't use the SDK and instead generate a raw HTTP
            request to the Secrets Manager service endpoint, then you must generate a
            ``ClientRequestToken`` yourself for the new version and include that value in the request.
          ...
          that you generate a `UUID-type <https://wikipedia.org/wiki/Universally_unique_identifier>`__
          value to ensure uniqueness of your versions within the specified secret.

          * If the ``ClientRequestToken`` value isn't already associated with a version of the secret then
          a new version of the secret is created.
        ...
        :param KmsKeyId:
          ...
          (Optional) Specifies the ARN, Key ID, or alias of the AWS KMS customer master key (CMK) to be
          used to encrypt the ``SecretString`` or ``SecretBinary`` values in the versions stored in this
          secret.

          You can specify any of the supported ways to identify a AWS KMS key ID. If you need to reference
        ...
        :param SecretString:
          ...
          the information as a JSON structure of key/value pairs that the Lambda rotation function knows
          how to parse.
        ...
        :param Tags:
          ...
          .. warning::
            ...
            * If you check tags in IAM policy ``Condition`` elements as part of your security strategy,
            then adding or removing a tag can change permissions. If the successful completion of this
            operation would result in you losing your permissions for this secret, then this operation is
          ...
          * If your tagging schema will be used across multiple services and resources, remember that other
          services might have restrictions on allowed characters. Generally allowed characters are:
    ...
    def delete_secret(
        ...
        during which you can restore the secret. If you don't specify a recovery window value, the
        operation defaults to 30 days. Secrets Manager attaches a ``DeletionDate`` stamp to the secret that
        specifies the end of the recovery window. At the end of the recovery window, Secrets Manager
        ...
        .. note::
          ...
          * There is no explicit operation to delete a version of a secret. Instead, remove all staging
          ...
          allows Secrets Manager to delete it as needed. Versions that do not have any staging labels do
          ...
          * The permanent secret deletion at the end of the waiting period is performed as a background
          task with low priority. There is no guarantee of a specific time after the recovery window for
          the actual delete operation to occur.
        ...
        **Request Syntax**
        ...
        :param ForceDeleteWithoutRecovery:
          ...
          before the operation completes. If you write code to delete and then immediately recreate a
          secret with the same name, ensure that your code includes appropriate back off and retry logic.
    ...
    def describe_secret(self, SecretId: str) -> ClientDescribeSecretResponseTypeDef:
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          The identifier of the secret whose details you want to retrieve. You can specify either the
          Amazon Resource Name (ARN) or the friendly name of the secret.
    ...
    def get_secret_value(
        ...
        **Request Syntax**
        ...
        :param VersionId:
          ...
          <https://wikipedia.org/wiki/Universally_unique_identifier>`__ value with 32 hexadecimal digits.
    ...
    def list_secret_version_ids(
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          The identifier for the secret containing the versions you want to list. You can specify either
        ...
        :param MaxResults:
          ...
          to the operation to get the next part of the results. Note that Secrets Manager might return
    ...
    def put_resource_policy(
        ...
        specify the secret's Amazon Resource Name (ARN) in the policy statement's ``Resources`` element.
        You can also use a combination of both identity-based and resource-based policies. The affected
        users and roles receive the permissions that are permitted by all of the relevant policies. For
        more information, see `Using Resource-Based Policies for AWS Secrets Manager
    ...
    def put_secret_value(
        ...
        """
        Stores a new encrypted secret value in the specified secret. To do this, the operation creates a
        new version and attaches it to the secret. The version can contain a new ``SecretString`` value or
        a new ``SecretBinary`` value. You can also specify the staging labels that are initially attached
        to the new version.
        ...
        * If this operation moves the staging label ``AWSCURRENT`` from another version to this version
        (because you included it in the ``StagingLabels`` parameter) then Secrets Manager also
    ...
    def rotate_secret(
        ...
        rotation with the values already stored in the secret. After the rotation completes, the protected
    ...
    def untag_resource(self, SecretId: str, TagKeys: List[str]) -> None:
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          The identifier for the secret that you want to remove tags from. You can specify either the
          Amazon Resource Name (ARN) or the friendly name of the secret.
    ...
    def update_secret(
        ...
        *either*  ``SecretString`` or ``SecretBinary`` then it also creates a new version attached to the
        ...
        .. note::
          ...
          version of a secret, you must use either the AWS CLI or one of the AWS SDKs.
        ...
        **Request Syntax**
        ...
        :param ClientRequestToken:
          ...
          You typically only need to interact with this value if you implement your own retry logic and
        ...
        :param SecretBinary:
          ...
          binary data in a file and then use the appropriate technique for your tool to pass the contents
          of the file as a parameter. Either ``SecretBinary`` or ``SecretString`` must have a value, but
    ...
    def update_secret_version_stage(
        ...
        version as it progresses through the secret rotation process. You can attach a staging label to
        only one version of a secret at a time. If a staging label to be added is already attached to
        another version, then it is moved--removed from the other version first and then attached to this
        ...
        list of staging labels--they don't replace it.
