"Helper functions for acm service"
...
def boto3_client(
    ...
    """
    Equivalent of `boto3.client('acm')`, returns a correct type.
    """
    kwargs: Dict[str, Any] = {}
    ...
    if session is not None:
        return session.client("acm", **kwargs)
...
# pylint: disable=arguments-differ,redefined-outer-name,redefined-builtin
def get_certificate_validated_waiter(client: Client) -> CertificateValidatedWaiter:
    """
    Equivalent of `client.get_waiter('certificate_validated')`, returns a correct type.
    """
    return client.get_waiter("certificate_validated")
