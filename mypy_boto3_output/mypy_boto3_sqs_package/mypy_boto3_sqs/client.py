...
class Client(BaseClient):
    ...
    def add_permission(
        ...
        the queue, can grant or deny permissions to the queue. For more information about these
        permissions, see `Allow Developers to Write Messages to a Shared Queue
    ...
    def change_message_visibility(
        ...
        ``ChangeMessageVisibility`` to extend the visibility timeout to the maximum allowed time. If you
        try to extend the visibility timeout beyond the maximum, your request is rejected.
        ...
        * Deleted from the queue.
        ...
        of approximately 120,000 inflight messages (received from a queue by a consumer, but not yet
        deleted from the queue). If you reach this limit, Amazon SQS returns the ``OverLimit`` error
        message. To avoid reaching the limit, you should delete messages from the queue after they're
        processed. You can also increase the number of queues you use to process your messages. To request
        a limit increase, `file a support request
        ...
        **Request Syntax**
        ...
        :param Attributes:
          ...
          * ``MaximumMessageSize`` - The limit of how many bytes a message can contain before Amazon SQS
          rejects it. Valid values: An integer from 1,024 bytes (1 KiB) to 262,144 bytes (256 KiB).
          ...
          * ``KmsMasterKeyId`` - The ID of an AWS-managed customer master key (CMK) for Amazon SQS or a
          ...
          . While the alias of the AWS-managed CMK for Amazon SQS is always ``alias/aws/sqs`` , the alias
          of a custom CMK can, for example, be ``alias/*MyAlias* `` . For more examples, see `KeyId
          ...
          * ``KmsDataKeyReusePeriodSeconds`` - The length of time, in seconds, for which Amazon SQS can
          ...
          decrypt messages before calling AWS KMS again. An integer representing seconds, between 60
          seconds (1 minute) and 86,400 seconds (24 hours). Default: 300 (5 minutes). A shorter time period
          provides better security but results in more calls to KMS which might incur charges after Free
          ...
          * ``FifoQueue`` - Designates a queue as FIFO. Valid values: ``true`` , ``false`` . If you don't
          ...
          attribute only during queue creation. You can't change it for an existing queue. When you set
          this attribute, you must also provide the ``MessageGroupId`` for your messages explicitly. For
          more information, see `FIFO Queue Logic
    ...
    def get_queue_attributes(
        ...
        **Request Syntax**
        ...
        :param AttributeNames:
          ...
          * ``ApproximateNumberOfMessagesDelayed`` - Returns the approximate number of messages in the
          queue that are delayed and not available for reading immediately. This can happen when the queue
          is configured as a delay queue or when a message has been sent with a delay parameter.
          ...
          * ``MaximumMessageSize`` - Returns the limit of how many bytes a message can contain before
    ...
    def receive_message(
        ...
        than you requested per ``ReceiveMessage`` call. If the number of messages in the queue is extremely
        small, you might not receive any messages in a particular ``ReceiveMessage`` response. If this
        ...
        **Request Syntax**
        ...
        :param MessageAttributeNames:
          ...
          * The name can be up to 256 characters long.
          ...
          return all of the attributes by specifying ``All`` or ``.*`` in your request. You can also use
          all message attributes starting with a prefix, for example ``bar.*`` .
        ...
        :param WaitTimeSeconds:
          ...
          messages are available and the wait time expires, the call returns successfully with an empty
          list of messages.
        ...
        :param ReceiveRequestAttemptId:
          ...
          a ``ReceiveMessage`` action, and instead of a response you receive a generic error, you can retry
          ...
          * You can retry the ``ReceiveMessage`` action with the same ``ReceiveRequestAttemptId`` if none
          ...
          * While messages with a particular ``MessageGroupId`` are invisible, no more messages belonging
          to the same ``MessageGroupId`` are returned until the visibility timeout expires. You can still
    ...
    def send_message(
        ...
        **Request Syntax**
        ...
        :param MessageGroupId:
          ...
          The tag that specifies that a message belongs to a specific message group. Messages that belong
          to the same message group are processed in a FIFO manner (however, messages in different message
    ...
    def set_queue_attributes(self, QueueUrl: str, Attributes: Dict[str, str]) -> None:
        ...
        **Request Syntax**
        ...
        :param Attributes: **[REQUIRED]**
          ...
          * ``MessageRetentionPeriod`` - The length of time, in seconds, for which Amazon SQS retains a
          message. Valid values: An integer representing seconds, from 60 (1 minute) to 1,209,600 (14
